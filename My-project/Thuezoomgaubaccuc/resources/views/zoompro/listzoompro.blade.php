          ...
          <div class="p-10 mx-auto max-w-7xl md:p-6">
            <div class="grid grid-cols-12 gap-4 md:gap-6">
              <div class="col-span-12 space-y-6 xl:col-span-12 ">
                <div class="">
                  <h1 class="font-bold !text-[24px]">Tài khoản zoom</h1>
                </div>
                <div>
                    <a href="{{ route('addzoompro') }}" class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600">
                        Thêm Mới
                    </a>
                </div>
                <div class="bg-white p-4 rounded-md shadow">
                  <form action="" method="get">
                    ...
                    <div class="flex flex-wrap gap-4">
                      ...
                      <div>
                          <label for="user" class="block text-sm font-medium text-gray-700 mb-2">Tên tài khoản</label>
                          <input 
                              id="user" 
                              name="user"
                              type="text" 
                  ...
                  <div class="overflow-x-auto">
                    ...
                    <table class="min-w-full bg-white border  rounded-lg shadow-md">
                      <thead class="">
                        <tr>
                          ...
                          <th class="px-4 py-2 border text-left">Mật khẩu</th>
                          <th class="px-4 py-2 border text-left">key host</th>
                          <th class="px-4 py-2 border text-center">Hành Động</th>
                        </tr>
                      </thead>
                      @foreach ($zoompros as $zoompro)
                        <tr class="hover:bg-gray-100">
                        <td class="border border-gray-300 px-4 py-2">{{ $zoompro->id }}</td>
                            <td class="border border-gray-300 px-4 py-2">{{ $zoompro->user }}</td>
                            <td class="border border-gray-300 px-4 py-2">{{ $zoompro->pass }}</td>
                            <td class="border border-gray-300 px-4 py-2">{{ $zoompro->key_host }}</td>
                            <td class="border border-gray-300 px-4 py-2 text-center">
                            <a href="{{ route('editzoompro', $zoompro->id) }}" class="bg-blue-500 px-3 py-1 rounded-md hover:bg-blue-600">
                                <i class="fa-solid fa-pencil"></i>
                                    ...
                                    |
                                    <form 
                                        action="{{ route('deletezoompro', $zoompro->id) }}" 
                                        method="POST" 
                                        class="inline"
                    ...
                    <div class="flex justify-center items-center mt-4">
                        <nav class="flex space-x-1">
                            @if ($zoompros->onFirstPage())
                                <span class="px-3 py-1 border border-gray-300 text-gray-500 rounded-md">«</span>
                            @else
                                <a href="{{ $zoompros->previousPageUrl() }}" class="px-3 py-1 border border-gray-500 text-gray-700 rounded-md hover:bg-gray-200">«</a>
                            @endif

                            @for ($i = 1; $i <= $zoompros->lastPage(); $i++)
                                <a href="{{ $zoompros->url($i) }}" class="px-3 py-1 border {{ $zoompros->currentPage() == $i ? 'border-blue-600 bg-blue-500 text-white' : 'border-gray-500 text-gray-700 hover:bg-gray-200' }} rounded-md">
                                    {{ $i }}
                                </a>
                            @endfor

                            @if ($zoompros->hasMorePages())
                                <a href="{{ $zoompros->nextPageUrl() }}" class="px-3 py-1 border border-gray-500 text-gray-700 rounded-md hover:bg-gray-200">»</a>
                            @else
                                <span class="px-3 py-1 border border-gray-300 text-gray-500 rounded-md">»</span>
                            @endif
                        </nav>
