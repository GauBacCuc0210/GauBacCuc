          ...
          <div class="p-10 mx-auto max-w-7xl md:p-6">
            <div class="grid grid-cols-12 gap-4 md:gap-6">
              <div class="col-span-12 space-y-6 xl:col-span-12 ">
                <div class="">
                  <h1 class="font-bold !text-[24px]">Danh sách cài đặt giá</h1>
                </div>
                <div>
                  ...
                  <div class="overflow-x-auto">
                    ...
                    <table class="min-w-full bg-white border  rounded-lg shadow-md">
                      <thead class="">
                        <tr>
                          ...
                          <th class="px-4 py-2 border text-left">Loại tài khoản</th>
                          <th class="px-4 py-2 border text-left">Giá</th>
                          <th class="px-4 py-2 border text-left">Giờ</th>
                          <th class="px-4 py-2 border text-center">Hành Động</th>
                        </tr>
                      </thead>
                      @foreach ($prices as $price)
                        <tr class="hover:bg-gray-100">
                            <td class="border border-gray-300 px-4 py-2">{{ $price->id }}</td>
                            <td class="border border-gray-300 px-4 py-2">{{ $price->type }}</td>
                            <td class="border border-gray-300 px-4 py-2">{{ number_format($price->price, 0, ',', '.') }} đ</td>

                            <td class="border border-gray-300 px-4 py-2">{{ $price->hour }}</td>
                            <td class="border border-gray-300 px-4 py-2 text-center">
                            <a href="{{ route('editprices', $price->id) }}" class="bg-blue-500 px-3 py-1 rounded-md hover:bg-blue-600">
                                <i class="fa-solid fa-pencil"></i>
                            </a>
                                    |
                                    <form action="{{ route('deleteprice', $price->id) }}" method="POST" 
                                        class="inline"
                    ...
                    <div class="flex justify-center items-center mt-4">
                        <nav class="flex space-x-1">
                            ...
                            @else
                                <a href="{{ $prices->previousPageUrl() }}" class="px-3 py-1 border border-gray-500 text-gray-700 rounded-md hover:bg-gray-200">«</a>
                            @endif

                            @for ($i = 1; $i <= $prices->lastPage(); $i++)
                                <a href="{{ $prices->url($i) }}" class="px-3 py-1 border {{ $prices->currentPage() == $i ? 'border-blue-600 bg-blue-500 text-white' : 'border-gray-500 text-gray-700 hover:bg-gray-200' }} rounded-md">
                                    {{ $i }}
                                </a>
                            @endfor

                            @if ($prices->hasMorePages())
                                <a href="{{ $prices->nextPageUrl() }}" class="px-3 py-1 border border-gray-500 text-gray-700 rounded-md hover:bg-gray-200">»</a>
                            @else
                                <span class="px-3 py-1 border border-gray-300 text-gray-500 rounded-md">»</span>
                            @endif
                        </nav>
