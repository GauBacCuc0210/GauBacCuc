                ...
                <div class="bg-white p-4 rounded-md shadow">
                  <form action="" method="get">
                    ...
                    <div class="flex flex-wrap gap-4">
                      ...
                      <div>
                          <label for="minuname" class="block text-sm font-medium text-gray-700 mb-2">Tên khách hàng</label>
                          <input 
                              id="minuname" 
                              name="minuname"
                              type="text" 
                              class="border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 w-full"
                              value="{{ request('minuname') }}"
                          >
                      </div>
                      <div>
                  ...
                  <div class="overflow-x-auto">
                    ...
                    <table class="min-w-full bg-white border  rounded-lg shadow-md">
                      ...
                      @foreach ($minus as $minu )
                        ...
                        <tr class="hover:bg-gray-100">
                            <td class="border border-gray-300 px-4 py-2">{{ $minu->id }}</td>
                            <td class="border border-gray-300 px-4 py-2">{{ $minu->time }}</td>

                            <td class="border border-gray-300 px-4 py-2">
                               
                                {{ number_format($minu->numbercoinmanager, 0) }} đ
                            </td>

                            <td class="border border-gray-300 px-4 py-2">
                            {{ $minu->user->name ?? 'Không có tên' }} <br>
                            {{ $minu->user->phone ?? 'Không có số điện thoại' }} <br>
                            {{ $minu->user->email ?? 'Không có email' }} <br>
                            {{ number_format($minu->numbercoin, 0) }} đ

                            </td>

                            <td class="border border-gray-300 px-4 py-2">{{ $minu->note }}</td>
                    ...
                    <div class="flex justify-center items-center mt-4">
                        <nav class="flex space-x-1">
                            @if ($minus->onFirstPage())
                                <span class="px-3 py-1 border border-gray-300 text-gray-500 rounded-md">«</span>
                            @else
                                <a href="{{ $minus->previousPageUrl() }}" class="px-3 py-1 border border-gray-500 text-gray-700 rounded-md hover:bg-gray-200">«</a>
                            @endif

                            @for ($i = 1; $i <= $minus->lastPage(); $i++)
                                <a href="{{ $minus->url($i) }}" class="px-3 py-1 border {{ $minus->currentPage() == $i ? 'border-blue-600 bg-blue-500 text-white' : 'border-gray-500 text-gray-700 hover:bg-gray-200' }} rounded-md">
                                    {{ $i }}
                                </a>
                            @endfor

                            @if ($minus->hasMorePages())
                                <a href="{{ $minus->nextPageUrl() }}" class="px-3 py-1 border border-gray-500 text-gray-700 rounded-md hover:bg-gray-200">»</a>
                            @else
                                <span class="px-3 py-1 border border-gray-300 text-gray-500 rounded-md">»</span>
                            @endif
                        </nav>
