            ...
            <div class="grid grid-cols-12 gap-4 md:gap-6">
                <div class="col-span-12 space-y-6 xl:col-span-12 ">
                    ...
                    <h2 class="text-xl font-bold mb-4">Nạp tiền cho: {{ $user->name }}</h2>
                       <form action="{{ route('user.topup.store', $user->id) }}" method="POST">
                            @csrf 
                            <div class="flex gap-4 mb-4">
                                <div class="w-1/2">
                                    <label for="amount" class="block text-sm font-medium text-gray-700 mb-2">Số tiền cộng cho tài khoản: {{ $user->name }}-
                                        Số dư: {{ $user->coin }}đ</label>
                            ...
                            <div class="flex gap-4">
                                <div class="w-1/2">
                                    <label for="note" class="block text-sm font-medium text-gray-700 mb-2">Lý do cộng</label>
                                    <input 
                                        name="note" 
                                        type="text" 
                                        class="border border-gray-300 rounded-md px-4 py-2 focus:ring-2 focus:ring-blue-500 w-full"
